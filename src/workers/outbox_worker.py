...
from src.repository.outbox_factory import OutboxRepositoryFactory
...
class OutboxWorker:
    def __init__(self, repo_factory: OutboxRepositoryFactory | None = None) -> None:
        self.publisher = KafkaProducer()
        self.repo_factory = repo_factory or OutboxRepositoryFactory(async_session_maker)
    ...
    async def claim_events(self, limit: int = 20) -> list:
        async with self.repo_factory() as repo:
            ...
            return [to_claimed_event(event, tokens[event.id]) for event in events]
